...
- **Buy cost per kg**: λₜ = goldₜ × rₜ × C_TROY  (K_CONV removed for the 2026
  forward schedule; weights have been recalibrated accordingly)
...
- **Objective** (per scenario, minimized):
  ```
  Σₜ [α·(Eₜ - p_sell·yₜ)₊ + β·(p_sell·yₜ - Eₜ)₊ - γ·λ̄·sₜ₊₁]  −  λ̄·s_T
  ```
  where `Eₜ = ρ·λₜ·Vₜ`. **No φ shortfall penalty** in the 2026 forward
  schedule — Q_ANNUAL is reported as a target only.
...
4. **No annual shortfall penalty** (2026 forward schedule): φ has been removed.
   The annual target Q_ANNUAL is reported (`shortfall_kg`, "scenarios meeting Q")
   but no GHS penalty is applied to the objective. This was deliberate — the
   2026 forward schedule prioritises optimal routing decisions under uncertainty
   rather than enforcing strict mandate compliance through penalty.
