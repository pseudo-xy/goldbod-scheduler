...
with st.sidebar:
    ...
    with st.expander("Conversion constants (advanced)"):
        c_troy = st.number_input(
            "C_TROY (troy oz/kg)",
            value=32.1507, step=0.0001, format="%.4f",
            help="Conversion factor used in λ = gold × fx × C_TROY."
        )
        st.caption(
            "Note: K_CONV is no longer used in the lambda formula for the 2026 "
            "forward schedule. The current formula is λ = gold × fx × C_TROY."
        )

    # ─── Objective weights ───
    st.subheader("Objective Weights")
    st.caption("Weights calibrated for the 2026 forward schedule (no K_CONV in λ).")
    alpha = st.number_input("α (under-release penalty)", value=3.195, min_value=0.0, step=0.01, format="%.3f")
    beta = st.number_input("β (over-release penalty)", value=2.413, min_value=0.0, step=0.01, format="%.3f")
    gamma = st.number_input("γ (intermediate inventory reward)", value=0.164, min_value=0.0, step=0.001, format="%.3f")
    # PHI removed — no shortfall penalty in the 2026 forward schedule.
    # Q_ANNUAL is now reported as a target only (no penalty applied).
...
params = DPParams(
    ...
    s_bar=s_bar,
    C_TROY=c_troy,
    # K_CONV no longer used in lambda formula
    alpha=alpha,
    beta=beta,
    gamma=gamma,
    # phi removed — no shortfall penalty in 2026
...
with preview_cols[0]:
    ...
    lambda_preview = gold_preview * fx_mid * c_troy
...
if 'res' in st.session_state:
    ...
    # ─── Second row: release routing summary (kg) ───
    ...
    st.markdown("##### GoldBod Financial Summary (mean across scenarios)")
    fin_cols = st.columns(3)
    with fin_cols[0]:
        st.metric(
            "Total Revenue (GHS)",
            f"{df_scen['Total_Revenue_GHS'].mean() / 1e9:.3f} bn",
            f"= Σ purchase_t × λ_t",
            help="Total value of gold purchased across the year: "
                 "each week's purchases multiplied by that week's buy cost (λₜ = gold price × FX × C_TROY), "
                 "summed over all 52 weeks.",
        )
    with fin_cols[1]:
        st.metric(
            "Total Inflow from BoG (GHS)",
            f"{df_scen['Total_Inflow_GHS'].mean() / 1e9:.3f} bn",
            f"= Σ release_BoG_t × λ_t",
            help="Total value of gold transferred to BoG's national reserve across the year. "
                 "Computed as each week's BoG release in kg × λₜ.",
        )
    with fin_cols[2]:
        st.metric(
            "GoldBod Charges / Expenses (GHS)",
            f"{df_scen['Total_Charges_GHS'].mean() / 1e9:.3f} bn",
            f"= {(1 - params.rho) * 100:.3f}% × BoG inflow",
            help=f"GoldBod's retained earnings: {(1-params.rho)*100:.3f}% of the total BoG inflow "
                 f"(= 1 − ρ = 1 − {params.rho}).",
        )
    ...
    with tabs[4]:
        ...
        mean_bog_value_ghs = df_scen['Total_BOG_Value_GHS'].mean()
        mean_mkt_value_ghs = df_scen['Total_Mkt_Value_GHS'].mean()
        ...
        # Row 2: GHS values of each routing destination
        st.markdown("##### Year-end GHS Values by Routing Destination")
        val_cols = st.columns(3)
        with val_cols[0]:
            st.metric(
                "Total Value to BoG Reserve (GHS)",
                f"{mean_bog_value_ghs / 1e9:.3f} bn",
                f"= {bog_pct:.1f}% of total release value",
                help="Σ release_BoG_t × λ_t across the full year.",
            )
        with val_cols[1]:
            st.metric(
                "Total Value to Market (GHS)",
                f"{mean_mkt_value_ghs / 1e9:.3f} bn",
                f"= {mkt_pct:.1f}% of total release value",
                help="Σ release_Market_t × λ_t across the full year.",
            )
        with val_cols[2]:
            total_release_value = mean_bog_value_ghs + mean_mkt_value_ghs
            st.metric(
                "Total Release Value (GHS)",
                f"{total_release_value / 1e9:.3f} bn",
                f"BoG + Market combined",
                help="Total GHS value of all gold released across both destinations.",
            )
    ...
    with tabs[6]:
        ...
        config = {
            ...
            "Q_ANNUAL (kg)": f"{q_annual:,.0f} (reporting only — no penalty)",
            "Supply ratio ρ": rho,
            "Working buffer S̄ (kg)": s_bar,
            "Weights (α, β, γ)": f"({alpha}, {beta}, {gamma}) — φ removed",
