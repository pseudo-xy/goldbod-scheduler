...
class DPParams:
    ...
    K_CONV: float = 4.186  # legacy — no longer used in lambda formula for 2026
    ...
    # Objective weights
    # Calibrated WITHOUT K_CONV in lambda formula (2026 forward schedule)
    alpha: float = 3.195
    beta: float = 2.413
    gamma: float = 0.164
    # phi removed — no shortfall penalty in 2026 forward schedule
...
def generate_scenarios(params: DPParams, gold_weekly: np.ndarray):
    ...
    # Buy cost per kg: gold * fx * C_TROY  (K_CONV removed for 2026)
    cost = gold_weekly[None, :] * fx * params.C_TROY
    inflow = cost * vol
...
def simulate_forward(
    ...
    GOLDBOD_CHARGE_RATE = 1.0 - params.rho   # = 0.00758
    ...
    total_revenue_ghs    = 0.0   # Σ x_t × λ_t  (total gold bought in GHS)
    total_bog_value_ghs  = 0.0   # Σ release_bog_t × λ_t
    total_mkt_value_ghs  = 0.0   # Σ release_market_t × λ_t

    for t in range(T):
        Et = target_xi[t]
        vol_t = vol_xi[t]
        fx_t = fx_xi[t]
        lam_t = p_sell_xi[t]   # λ_t = gold_t × fx_t × C_TROY
        ...
        # Financial values this period
        total_revenue_ghs   += x_pol   * lam_t
        total_bog_value_ghs += y_bog   * lam_t
        total_mkt_value_ghs += y_market * lam_t
    ...
    # Total inflow from BoG = value sent to BoG reserve
    total_inflow_ghs = total_bog_value_ghs
    # GoldBod charges = 0.00758 × total inflow
    total_charges_ghs = GOLDBOD_CHARGE_RATE * total_inflow_ghs

    # Shortfall for reporting only — no PHI penalty in 2026
    shortfall_kg = max(Q_ANNUAL - cum_purch, 0.0)
    traj['shortfall_kg']        = shortfall_kg
    traj['shortfall_ghs']       = 0.0
    traj['final_inv']           = s
    traj['total_revenue_ghs']   = total_revenue_ghs
    traj['total_bog_value_ghs'] = total_bog_value_ghs
    traj['total_mkt_value_ghs'] = total_mkt_value_ghs
    traj['total_inflow_ghs']    = total_inflow_ghs
    traj['total_charges_ghs']   = total_charges_ghs
    return traj
...
def solve_stochastic_dp(params: DPParams, progress_callback=None) -> Dict[str, Any]:
    ...
    lambda_arr = gold_weekly * fx_mid * params.C_TROY
    ...
    for xi in range(n_scen):
        lambda_xi = gold_weekly * fx_scen[xi] * params.C_TROY
        ...
        # GHS value of each routing destination: Σ (release_t × λ_t)
        bog_arr = np.array(traj_xi['release_bog'])
        mkt_arr = np.array(traj_xi['release_market'])
        bog_value_ghs = float(np.dot(bog_arr, lambda_xi))
        mkt_value_ghs = float(np.dot(mkt_arr, lambda_xi))
        ...
        results.append({
            ...
            'Total_BOG_Value_GHS': bog_value_ghs,
            'Total_Mkt_Value_GHS': mkt_value_ghs,
            ...
            # Financial metrics
            'Total_Revenue_GHS':    traj_xi['total_revenue_ghs'],
            'Total_BOG_Value_GHS':  traj_xi['total_bog_value_ghs'],
            'Total_Mkt_Value_GHS':  traj_xi['total_mkt_value_ghs'],
            'Total_Inflow_GHS':     traj_xi['total_inflow_ghs'],
            'Total_Charges_GHS':    traj_xi['total_charges_ghs'],
        })
